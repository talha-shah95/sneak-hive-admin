...
export const editProductValidationSchema = Yup.object().shape({
  ...
  pros: Yup.array()
    ...
    .test('non-empty-pros', 'At least one pro must have content', (pros) => {
      if (!pros || pros.length === 0) return false;
      return pros.some((pro) => pro && pro.trim().length > 0);
    }),
  cons: Yup.array()
    .of(
      Yup.string()
    ...
    .test('non-empty-cons', 'At least one con must have content', (cons) => {
      if (!cons || cons.length === 0) return false;
      return cons.some((con) => con && con.trim().length > 0);
    }),
