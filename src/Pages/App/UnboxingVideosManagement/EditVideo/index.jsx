...
import EditVideoService from './Services/EditVideoService';
import GetBrands from '../../BrandManagement/Services/GetBrands';
import GetVideo from '../VideoDetails/Services/GetVideo';

import { editVideoValidationSchema } from './Validations';
...
const EditVideo = () => {
  ...
  const { mutate: editVideoMutation, isLoading } = useForm({
    ...
    onError: (error) => {
      showToast(error?.data?.message.failed || 'Video edit failed', 'error');
    },
  });
  ...
  const handleSubmit = (values) => {
    ...
    if (thumbnailImage) {
      formDataToSend.append('thumbnail_file', thumbnailImage);
    }
    ...
    showModal({
      ...
      modalProps: {
        title: 'Update Video?',
        message: 'Are you sure you want to update the Video?',
        continueText: 'Yes',
        cancelText: 'No',
        onContinue: async () => {
          editVideoMutation({
            service: EditVideoService,
                  ...
                  <>
                    <Formik
                      initialValues={{
                        ...
                        brand_id:
                          videoDetailsData?.brand_id ||
                          brandList[0]?.value ||
                          selectedBrand ||
                          '',
                        description: videoDetailsData?.description || '',
                        affiliate_link: videoDetailsData?.affiliate_link || '',
                        is_active: videoDetailsData?.is_active || 1,
                        thumbnail_file: videoDetailsData?.thumbnail || '',
                        video_file: videoDetailsData?.video || '',
                      }}
