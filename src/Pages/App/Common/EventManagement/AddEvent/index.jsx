...
import { addEventValidationSchema } from './Validations';
...
const AddEvent = () => {
  ...
  const [includesAlcohol, setIncludesAlcohol] = useState(true);

  const { mutate: addEventMutation, isLoading } = useForm({
  ...
  useEffect(() => {
    if (eventTypes && !selectedEventType) {
      setSelectedEventType(eventTypes[0].value);
    }
